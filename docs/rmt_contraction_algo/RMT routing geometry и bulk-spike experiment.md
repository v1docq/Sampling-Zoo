...
|---|---|---|
| B0 | Обычные partition-модели, `uniform` и A9/A10 | Воспроизводимый безопасный базовый вариант. |
| B1 | Один bulk expert и один aggregate spike expert | Прямая проверка двухкомпонентной гипотезы. |
| B2 | Один bulk expert и несколько experts по spike signatures | Проверка многокомпонентной иерархии сигнала. |
| B3 | Выбор B0/B1/B2 по внутренним фолдам | Основная политика с обязательным возвратом к B0. |
...
Основная гипотеза подтверждается только если B3 выбирает B1 или B2 и при этом:
...
9. [x] Запустить первичную проверку A9 на новых датасетах и восстановить контрфактические решения A8 из оценок на тех же внутренних фолдах.
10. [x] Выполнить независимую регрессионную проверку A9 с заранее
    зафиксированным машиночитаемым критерием допуска.
11. [x] Реализовать component split, участие строк и exact-budget topology.
12. [x] Выполнить синтетический критерий bulk/spike: 1260 из 1260 запусков,
    строгий селектор прошёл проверку.
13. [x] Реализовать B0--B3 screening с LightGBM, полнообъёмный эталон и
    инкрементальные артефакты.
14. [ ] Запустить пилот на четырёх реальных задачах: 320 записей.
15. [ ] Выполнить независимое подтверждение на восьми новых задачах:
    640 записей.
16. [ ] После двух пройденных критериев запустить полную AMLB-сетку:
    1200 записей.
17. [ ] На восьми задачах выполнить плотную сетку из десяти бюджетов,
    аппроксимировать кривые с полнообъёмным якорем и построить итоговый отчёт.

Машиночитаемый порядок находится в
`docs/experiments/rmt_research_program_plan_20260812/research_program_plan.json`.
Первой точкой серверного запуска является классификационный критерий
безопасности, затем Phase S.1 и реальный bulk/spike-пилот.
