...
Задача - построить **новый RMT/tensor-contraction sampler**, который:
...
Более корректная формулировка идеи такая - строить не набор ковариаций, а набор **случайных feature contractions / random views**:
...
Это хорошо согласуется с contraction-подходом, а по сути каждый $Z_j = X \Omega_j$ это отдельный contraction. В работах по случайным тензорам подчёркивается, что матричные slices/unfoldings сами по себе недостаточны, а полезнее изучать семейство таких contractions, которое переводит тензорную задачу в матричные спектральные задачи . 
